...
static inline int64_t get_bool_vect(my_vector<int64_t> const &a, size_t i, bool q)
{
    return (i < a.size() ? a[i] : q);
}

static inline void delete_zeroes(my_vector<int64_t> &a)
{
    while (a.size() > 0 && !a.back())
        a.pop_back();
}

static inline void check_vector(my_vector<int64_t> &a, size_t require)
{
...
static inline void sub(big_integer &a, big_integer const &b)
{
    ...
    {
        check_vector(a.data, i);
        a.data[i] -= (i < size(b) ? b.data[i] : 0) + carry;
        carry = 0;
        if (a.data[i] < 0)
        {
            a.data[i] += BASE;
...
{
    pos = other.pos;
    data = my_vector<int64_t>(other.data);
    return *this;
}
...
big_integer &big_integer::operator*=(big_integer const &rhs)
{
    ...
    for (size_t i = 0; i < sz_t; i++)
    {
        ...
        {
            t = ans.data[i + j] + data[i] * (j < sz_b ? rhs.data[j] : 0) + carry;
            ans.data[i + j] = (t % BASE);
            carry = t / BASE;
        }
    }
