...
private:
    ...
    auto&& arg_get(old_Arg& old_arg, new_Args&...)
    {
        return old_arg;
    };
    
    template <typename Fn, typename... old_Args, typename... new_Args>
    auto arg_get(bind_t<Fn, old_Args...>& b, new_Args&... new_args)
    {
        return b(new_args...);
    }
    
    template <int N, typename... new_Args>
    auto&& arg_get(place_holder<N>&, new_Args&... new_args)
    {
...
{
    return bind_t<F, Args...>(std::forward<F>(func), std::move(args)...);
}
